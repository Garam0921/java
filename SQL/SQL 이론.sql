...
-- monthname 월을 영문으로
-- weekday 요일을 정수로 (mySql의 경우 : 월요일이 0 ~ 일요일이 6)
select now(),
	last_day(now()),
	dayofyear(now()),
	monthname(now()),
...
-- if (조건식, 참인 경우, 거짓인 경우)
select if(125000 * 450 > 500000, '초과달성', '미달성');
...
-- case
-- 'case when 조건 실행' 의 형태로 조건에 따른 실행을 여러개 설정가능
-- 조건의 마지막에 나머지를 의미하는 else 사용(사용하지 않아도 됨)
-- case문의 마지막을 의미하는 end 사용(반드시 사용해야함!!)
